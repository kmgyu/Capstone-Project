...
document.addEventListener("DOMContentLoaded", () => {
    ...
    //     // 클릭한 날짜 설정
    //     calendarModal.querySelector('.clicked-date strong').innerText = date;
    ...
    //     const clickedDate = '2024-12-10'; // 클릭한 날짜 예시
    //     openCalendarModal(clickedDate);
    // });

    // // 모달 닫기 버튼 동작
    // document.getElementById('close-calendar-modal-btn').addEventListener('click', () => {
    //     closeCalendarModal();
    // });

    // // 할 일 추가 버튼
    // document.getElementById('add-calendar-task-btn').addEventListener('click', () => {
    //     alert('할 일을 추가합니다!');
    //     // 실제 추가 로직 구현
    // });

    // // 할 일 삭제 버튼
    // document.getElementById('delete-calendar-task-btn').addEventListener('click', () => {
    //     alert('할 일을 삭제합니다!');
    //     // 실제 삭제 로직 구현
    // });
    ...
    if (calendarEl) {
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: [
                { title: '작업1 이름', start: '2024-12-10', type: 'today' },
                { title: '작업2 이름', start: '2024-12-10', type: 'today' },
                { title: '이건 병충해', start: '2024-12-10', type: 'recommend' },
                { title: '작업1 이름', start: '2024-12-11', type: 'today' },
                { title: '병충쓰', start: '2024-12-11', type: 'recommend' },
              ],
            dayCellContent: function (arg) {
               // 날짜를 로컬 기준으로 YYYY-MM-DD로 변환
                const formattedDate = arg.date.toISOString().split('T')[0]; // 로컬 시간으로 변경

                // 해당 날짜의 이벤트 가져오기
                const events = calendar.getEvents().filter(event => {
                    const eventDate = new Date(event.start); // 이벤트의 시작 날짜
        ...
        function openCalendarModal(date) {
            ...
            // 날짜에 해당하는 이벤트 필터링
            const todayTasks = calendar.getEvents().filter(event => event.startStr === date && event.extendedProps.type === 'today');
            const recommendTasks = calendar.getEvents().filter(event => event.startStr === date && event.extendedProps.type === 'recommend');
